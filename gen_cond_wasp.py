...
from model.wasp.multi_scale_ori_cond import MSResNet
...
for j in range (0, 16):
    ...
    for i in range (0, 12):
        print(i)
        idx = torch.Tensor([i])
        sampled_seq = diffusion.sample(batch_size=1, cond=idx)
        # sampled_seq = sampled_seq.view(-1)
        sample_seq_predict = (sampled_seq - sampled_seq.min()) / (sampled_seq.max() - sampled_seq.min())
        sample_seq_predict = 2. * sample_seq_predict - 1.
        # sample_seq_predict = torch.unsqueeze(sample_seq_predict, 1)
        idxV = torch.Tensor([i]).int().cuda()
        idxV = torch.unsqueeze(idxV, 1)
        # print(sample_seq_predict.shape)
        # print(idxV.shape)

        # Predict amplitude and shift
        predict_data = waspnet(sample_seq_predict, idxV)
        predict_amp = predict_data[:, 0]
        predict_shift = predict_data[:, 1]

        rc_sample_seq_predict = predict_amp * sample_seq_predict + predict_shift
        all_sampled_seqs.append(rc_sample_seq_predict.detach().cpu().numpy())

    final_data = np.array(all_sampled_seqs)
    np.save(save_path + 'final_result_wasp_' + str(j).zfill(2) + '.npy', arr=final_data)